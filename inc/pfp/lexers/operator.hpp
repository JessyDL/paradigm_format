...
class operator_state_t : public tokenizer_state_t {
  ...
  public:
	DEBUG_STATE_NAME(operator_state_t)
	token_candidate_t handle(std::string_view input, std::size_t start) override {
		...
		if(i + 1 < input.size()) {
			...
			if(multiCharOps.find(twoCharOp) != multiCharOps.end()) {
				return {token_t {twoCharOp, token_t::type_t::symbol}, 2, DEBUG_INJECT_STATE_NAME()};
			}
		}
		...
		if(i < input.size()) {
			...
			if(oneCharOps.find(oneCharOp) != oneCharOps.end()) {
				return {token_t {oneCharOp, token_t::type_t::symbol}, 1, DEBUG_INJECT_STATE_NAME()};
			}
		}
